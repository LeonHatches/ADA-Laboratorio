...
class AlgoritmoOptimizacionTareas:
    ...
    def algoritmo_camino_corto_secuenciacion(tareas: List[Tarea]) -> List[Tarea]:
        """CAMINO CORTO: Lógica de seucnecia para nuestros 4 tipos de tareas"""
        
        def costo_transicion_mejorado(tipo1: str, tipo2: str) -> int:
            """
            Costo basado en cambio de estados:
  
            1 = Mismo estado (transición suave)
            2 = Transición natural (descanso activo)  
            5 = Cambio brusco de estado
            """
            ...
            # MISMO ESTADO FÍSICO (costo 1)
            # estudio y organizacion
            if (tipo1 in ["estudio", "organizacion"] and 
                tipo2 in ["estudio", "organizacion"]):
                return 1
                
            # Activo: ejercicio y personal  
            if (tipo1 in ["ejercicio", "personal"] and 
                tipo2 in ["ejercicio", "personal"]):
                return 1
        ...
        while tareas_restantes:
            ...
            for i, tarea in enumerate(tareas_restantes):
                costo = costo_transicion_mejorado(ultima_tarea.tipo, tarea.tipo)
...
class SistemaGestionTareas:
    ...
    def generar_plan_optimizado(self) -> Dict[str, List[Tarea]]:
        ...
        print("PASO 1: GREEDY seleccionando tareas...")
        tareas_greedy = self.algoritmo_optimizacion.algoritmo_greedy_seleccion(self.lista_tareas)
        print(f"   Tareas seleccionadas: {[t.nombre for t in tareas_greedy]}")
    
        print("PASO 2: PROGRAMACIÓN DINÁMICA optimizando...")
        tareas_optimas = self.algoritmo_optimizacion.programacion_dinamica_optimizacion(tareas_greedy)
        print(f"   Tareas optimizadas: {[t.nombre for t in tareas_optimas]}")
    
        print("PASO 3: CAMINO CORTO ordenando secuencia...")
        secuencia_final = self.algoritmo_optimizacion.algoritmo_camino_corto_secuenciacion(tareas_optimas)
        print(f"   Secuencia final: {[t.nombre for t in secuencia_final]}")
...
class InterfazSistemaGestionTareas:
    ...
    def configurar_lista_tareas_moderna(self, parent):
        ...
        btn_eliminar = tk.Button(frame_botones, text="Eliminar Seleccionada",
